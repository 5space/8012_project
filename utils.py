...
def load_solution(sim, name):
    if name == "Euler 1":
        sim.set_body(0, 1, [0, 0, 0], [0, 0, 0])
        sim.set_body(1, 1, [1, 0, 0], [0, np.sqrt(1.25*sim.G), 0])
        sim.set_body(2, 1, [-1, 0, 0], [0, -np.sqrt(1.25*sim.G), 0])
        ...
        r2, r3 = r1+1, r1+1+z
        sim.set_body(0, m1, [r1, 0, 0], [0, r1*w, 0])
        sim.set_body(1, m2, [r2, 0, 0], [0, r2*w, 0])
        sim.set_body(2, m3, [r3, 0, 0], [0, r3*w, 0])
        ...
        sim.set_body(0, m1, r1, np.cross(ZHAT, r1*w))
        sim.set_body(1, m2, r2, np.cross(ZHAT, r2*w))
        sim.set_body(2, m3, r3, np.cross(ZHAT, r3*w))
    
    elif name == "Figure-8":
        sim.set_body(0, 1, [0.97000436, -0.24308753, 0], [0.466203685, 0.43236573, 0])
        sim.set_body(1, 1, [-0.97000436, 0.24308753, 0], [0.466203685, 0.43236573, 0])
        sim.set_body(2, 1, [0, 0, 0], [-0.93240737, -0.86473146, 0])
        sim.G = 1
